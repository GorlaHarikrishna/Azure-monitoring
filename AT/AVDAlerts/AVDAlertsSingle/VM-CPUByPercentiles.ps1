...
foreach ($vm in $vms)
{
    ...
    Add-AzMetricAlertRuleV2 -Name $alertRuleName -ResourceGroupName $Parameters.resourceGroupName -WindowSize 00:10:00 -Frequency 00:10:00 -TargetResourceId $vm.Id -Condition $condition -ActionGroupId $Parameters.actionGroupId -Severity 4 -Description "Client Name:$Customer-The percentage of CPU utilization on VM $($vm.Name)"
}